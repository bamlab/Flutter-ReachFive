...
class FlutterReachFiveIOS extends FlutterReachFivePlatform {
  ...
  Never parseError(Object error, StackTrace stackTrace) {
    if (error is PlatformException) {
      ...
      } else if (error.code == errorCodesInterface.updateSamePassword) {
      ...
      } else if (error.code == errorCodesInterface.unauthorizedRefreshToken) {
      ...
      } else if (error.code == errorCodesInterface.tooManyAttempts) {
        throw TooManyAttemptsExceptionInterface(error.message ?? "");
      } else if (error.code == errorCodesInterface.passwordPolicyError) {
        throw PasswordPolicyExceptionInterface(error.message ?? "");
      } else if (error.code ==
          errorCodesInterface.accountTemporarilySuspended) {
